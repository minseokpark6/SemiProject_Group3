...
colors = ["#336666", "#33cc99", '#D56062','#067BC2']

ver = 0   #0 : origin 1 : dark version
...
for 대여량,반납량 in bar_charts_data1:
    bar1 = ax.bar(0-bar_width/2, 대여량, bar_width, label='출근대여량', color=colors[0])
    bar2 = ax.bar(0+bar_width/2, 반납량, bar_width, label='출근반납량', color=colors[1])
    ax.set_ylim(0,28000)
    ax.set_xlim(-2,2)
    ax.set_xticks([])
    ax.set_yticks([])
...
for 대여량,반납량 in bar_charts_data2:
    bar1 = ax.bar(0-bar_width/2, 대여량, bar_width, label='퇴근대여량', color=colors[0])
    bar2 = ax.bar(0+bar_width/2, 반납량, bar_width, label='퇴근반납량', color=colors[1])
    ax.set_ylim(0,28000)
    ax.set_xlim(-2,2)
    ax.set_xticks([])
    ax.set_yticks([])
...
for sizes in pie_charts_data1:
    ax.pie(sizes, colors=(colors[0], colors[1]))
    buff = io.StringIO()
...
bicycle_group = folium.FeatureGroup(name='5. 따릉이 대여소')
...
if ver == 0:
    ...
    for lat1, lon1, 역명 in zip(지하철lonlat['convY'], 지하철lonlat['convX'], 지하철lonlat['역명']):
        subway_marker = folium.Marker(
            ...
            tooltip=역명,
            popup=역명,
            icon=folium.Icon(color='dodgerblue', icon_color='gray', icon='subway', angle=0, prefix='fa'),
            )
        subway_marker.add_to(subway_group)
...
강서따릉이.save('Data/00_지도_pie_bar_ver2_test.html')
